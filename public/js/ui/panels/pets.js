...
export function petsPanel({ state, actions }) {
  return {
    ...
    mount(body) {
      ...
      const note = h('p', { class: 'panel-note' }, 'Merge 3 identical pets into the next cosmetic tier. Tier 3 is the maximum; abilities stay the same.');
      ...
      function update() {
        blocks.replaceChildren(...BLOCKS.map((block) => {
          ...
          return h('div', { class: 'block-card', style: { '--bc': block.color } },
        ...
        const cards = owned.flatMap((pet) => [1, 2, 3].filter((tier) => profile.petTiers[pet.id]?.[tier]).map((tier) => {
          ...
          return h('div', { class: `item-card tier-${tier}${equipped ? ' equipped' : ''}`, style: { '--rc': RARITIES[pet.rarity].color } },
            ...
            h('button', { type: 'button', class: 'btn small block purple', disabled: tier === 3 || count < 3, onClick: () => actions.mergePet(pet.id, tier) }, tier === 3 ? 'Maximum tier' : `Merge 3 → Tier ${tier + 1}`),
